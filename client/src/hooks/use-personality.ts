...
async function fetchLatestPersonality(getToken: () => Promise<string | null>) {
    ...
    const res = await fetch(getApiUrl("/api/personality"), {
...
async function fetchPersonalityVersions(params: {
    ...
    const res = await fetch(getApiUrl(`/api/personality/versions?limit=${limit}`), {
        headers: token ? { Authorization: `Bearer ${token}` } : undefined,
    });
...
async function fetchPersonalityVersionById(params: {
    ...
    const res = await fetch(getApiUrl(`/api/personality/versions/${params.id}`), {
        headers: token ? { Authorization: `Bearer ${token}` } : undefined,
    });
